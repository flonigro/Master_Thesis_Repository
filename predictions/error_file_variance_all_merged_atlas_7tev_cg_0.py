...
#********************************************************************EXTRACT ERRORS and SYMMETRIZE ASYM*****************************************************
import yaml
...
def write_errors_and_variances_to_csv(errors_by_value, table_number):
    ...
    errors_output_file_path = f"/home/turing/MG5_aMC_v3_4_2/ATLAS_DIJET_7TEV_SCRIPTS_CG_0_NP_eq_0/{table_number}/errNew_file.csv"
    variance_output_file_path = f"/home/turing/MG5_aMC_v3_4_2/ATLAS_DIJET_7TEV_SCRIPTS_CG_0_NP_eq_0/{table_number}/varianceNewTable{table_number}.csv"
...
def calculate_variance(errors_list):
    ...
    for errors in errors_list:
        ...
        variance = np.sum([(e/100 - mean_error) ** 2 for e in errors]) / len(errors_list)
...
def save_mjj_high_values(table_number, label, high_values):
    ...
    with open(file_path, 'w', newline='') as csvfile:
        ...
        for name, high_value in high_values:
            adjusted_high_value = high_value*1000
...
def save_mjj_low_values(table_number, label, low_values):
    ...
    with open(file_path, 'w', newline='') as csvfile:
        ...
        for name, low_value in low_values:
            adjusted_low_value = low_value*1000
...
def main():
    for table_number in range(7, 13):
        ...
        label = labels[table_number - 7]
        file_path = os.path.join(table_directory, f'valuesReal_NEW_NEW_NEW{label}.csv')
        table_values = []
        high_values = []
        low_values = []

        file_path_yaml = os.path.join(directory, f'HEPData-ins1268975-v1-Table_{table_number}.yaml')
