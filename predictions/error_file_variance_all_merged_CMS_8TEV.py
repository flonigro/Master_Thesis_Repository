...
directory = '/home/turing/MG5_aMC_v3_4_2/dijet/data/rawdata/CMS_2JET_8TEV_rawdata/'
labels = ["0.0<y*<1.0", "1.0<y*<2.0", "2.0<y*<3.0", "0.0<y*<1.0", "1.0<y*<2.0", "0.0<y*<1.0"]
output_base_dir = '/home/turing/MG5_aMC_v3_4_2/CMS_DIJET_8TEV_SCRIPTS_CG_0_NP_eq_0/'
            ...
            if value_key is not None:
                ...
                for error in value.get('errors', []):
                    ...
                    if 'symerror' in error and 'stat' in error.get('label', ''):
                        ...
                        errors_by_value[value_key].append(float(error['symerror'].rstrip('%')))
                    elif 'asymerror' in error:
                        asym_error_minus = -float(error['asymerror']['minus'].rstrip('%'))
                        asym_error_plus = float(error['asymerror']['plus'].rstrip('%'))
                        semi_value = (asym_error_plus +  asym_error_minus) * 0.5
                        average = (asym_error_plus - asym_error_minus) * 0.5
                        errors_by_value[value_key].append(sqrt(average**2 + 2*(semi_value)**2))
...
def write_errors_and_variances_to_csv(errors_by_value, table_number):
    ...
    errors_output_file_path = f"/home/turing/MG5_aMC_v3_4_2/CMS_DIJET_8TEV_SCRIPTS_CG_0_NP_eq_0/{table_number}/errNew_file.csv"
    variance_output_file_path = f"/home/turing/MG5_aMC_v3_4_2/CMS_DIJET_8TEV_SCRIPTS_CG_0_NP_eq_0/{table_number}/varianceNewTable{table_number}.csv"
...
def calculate_variance(errors_list):
    ...
    for errors in errors_list:
        ...
        variance = variance = np.sum([((e/100) - mean_error/100) ** 2 for e in errors]) / len(errors_list)
...
def process_yaml_files_in_directory(directory_path):
    for filename in os.listdir(directory_path):
        if filename.endswith('.yaml'):
            ...
            table_number = int(filename.split('table_')[1].split('.')[0])
...
def main():
    for table_number in range(1, 7):
        ...
        label = labels[table_number - 1]
        file_path = os.path.join(table_directory, f'valuesReal_NEW_NEW_NEW{label}.csv')
        table_values = []
        high_values = []
        low_values = []

        file_path_yaml = os.path.join(directory, f'table_{table_number}.yaml')
