...
def plot_cross_sections_vs_mjj_with_global_uncertainties(csv_file_path_extracted, csv_file_path_real, csv_file_path_diff,label):
    ...
    df_extracted = pd.read_csv(csv_file_path_real)
    df_real = pd.read_csv(csv_file_path_extracted)
    ...
    axs[1].set_xscale
    axs[1].set_yscale
    axs[1].errorbar(df_real['Mean'], ((df_extracted['Cross Section']+df_real['Cross Section'])/df_real['Cross Section']),linestyle='None', marker='^', capsize=3, label="Ratio")
...
def plot_correlation(csv_file_path_real,csv_file_path_extracted, label):
    ...
    sns.heatmap((df_extracted['Cross Section'], df_real['Cross Section']), annot=False, cmap='coolwarm', cbar_kws={'label': 'Correlation'})
    plt.title(f'Correlation between Cross Section and MJJ ({label})')
    plt.xlabel(df_extracted['Cross Section'])
...
def main():
    ...
    for table in tables:
        df_sm = f"/home/turing/MG5_aMC_v3_4_2/ATLAS_DIJET_7TEV_SCRIPTS_CG_0_NP_eq_0/Table{table}/cross_section_output.csv"
        df_merged_sm  = f"/home/turing/MG5_aMC_v3_4_2/ATLAS_DIJET_7TEV_SCRIPTS_CG_0_NP_eq_0/Table{table}/merged_output_sorted_NEW.csv"
        df_smeft =f"/home/turing/MG5_aMC_v3_4_2/ATLAS_DIJET_7TEV_SCRIPTS_CG_1_NP_2_eq_2/Table{table}/cross_section_output_sorted.csv"
        label = None
        if table == 7:
            label = "0.0_y__0.5"
        elif table == 8:
...
********************************************************************************************"""
        ...
        k_factors_df = k_factors( f"/home/turing/MG5_aMC_v3_4_2/ATLAS_DIJET_7TEV_SCRIPTS_CG_1_NP_2_eq_2/Table{table}/", df_sm, df_smeft)
        clean_k_factors_df = remove_header_from_csv(f"/home/turing/MG5_aMC_v3_4_2/ATLAS_DIJET_7TEV_SCRIPTS_CG_1_NP_2_eq_2/Table{table}/k_factors.csv", f"/home/turing/MG5_aMC_v3_4_2/ATLAS_DIJET_7TEV_SCRIPTS_CG_1_NP_2_eq_2/Table{table}/clean_k_factors.csv")
        all_k_factors_df = pd.concat([all_k_factors_df, clean_k_factors_df])
        plot_cross_sections_vs_mjj_with_global_uncertainties(df_merged_sm, df_smeft, df_diff, label)
    ...
    with open(os.path.join(f"/home/turing/MG5_aMC_v3_4_2/K_FACTORS_ATLAS_DIJET_7TEV_NP_EQ_0_NP_2_EQ_2/all_k_factors.txt"), "w") as outfile:
